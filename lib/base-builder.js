...
class NodeBuilderBase {
    ...
    _applyNodeFields() {
        ...
        const compByNode = {};
        for (const [k, idx] of Object.entries(this._compMap)) {
            const [nn] = k.split('|');
            if (!compByNode[nn]) compByNode[nn] = [];
            compByNode[nn].push(idx);
        }
        const newCompByNode = {};
        for (const k of this._newCompKeys) {
            const [nn] = k.split('|');
            if (!newCompByNode[nn]) newCompByNode[nn] = [];
            newCompByNode[nn].push(this._compMap[k]);
        }
    ...
    _validate() {
        ...
        function deepCheck(obj, objPath) {
            if (!obj || typeof obj !== 'object') return;
            for (const [k, v] of Object.entries(obj)) {
                if (k === '__id__' && typeof v === 'number' && (v < 0 || v >= arr.length))
                    errors.push(`BAD REF: ${objPath}.${k}=${v} (max ${arr.length - 1})`);
                else if (Array.isArray(v)) v.forEach((it, i) => deepCheck(it, objPath + '[' + i + ']'));
                else if (typeof v === 'object' && v !== null) deepCheck(v, objPath + '.' + k);
            }
        }
        deepCheck(arr, 'root');
